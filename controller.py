...
class Controller(object):
    """ Controller built based on LSTM """
    def __init__(self, input_size, output_size, word_size, read_heads=1, shift_range=1, batch_szie=1):
        """
        Parameters:
        ------------
        ...
        with tf.name_scope("lstm_controller"):
            ...
            self.lstm_state = tf.Variable(tf.zeros([self.batch_size, 100]), trainable=False)
            self.lstm_output = tf.Variable(tf.zeros([self.batch_size, 100]), trainable=False)
            ...
            with tf.variable_scope("shape_inference"):
                ...
                output_ = self.lstm_cell(inputs=input_, state=(self.lstm_output, self.lstm_state))
                shape = output_.get_shape().as_list()
    ...
    def process_step_input(self, input_, last_read_vectors, state=None):
        """
        Processes input data (for each loop) through the Controller and
        ...
        input_concat_read_vector = tf.concat(values=[input_, flat_read_vectors], axis=1)

        nn_output, nn_state = self.lstm_cell(input_concat_read_vector, state)
    ...
    def parse_interface_vector(self, interface_vector):
        ...
        parsed['add_vector'] = tf.reshape(interface_vector[:, erase_end:write_end], write_shape)

        return parsed


    def get_state(self):
        return self.lstm_output, self.lstm_state
